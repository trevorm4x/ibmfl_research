...
class Runner:
    ...
    def __dump_trial_metadata(trial_info, config_agg_dict, config_party_dicts, ts, trial_cur):
        ...
        with open('{}/{}/trial{}/metadata.yml'.format(trial_info['local_staging_dir'],
                                                      ts,
                                                      trial_cur), "w") as mdfile:
    ...
    def __exec_command_sync(client, cmdstr):
        ...
        if status != 0:
            ...
            sys.exit(f'Error executing command \'{cmdstr}\' on remote {ip} with exit code {status}!\nstdout: \'{outstr}\'\nstderr: \'{errstr}\'')
    ...
    def __copy_config_to_server(config_dict, config_path_loc, client, config_path_rem):
        ...
        (status, outstr) = Runner.__exec_command_sync(client,
                                                      'mkdir -p {}'.format(config_path_rem_dir))
        if status != 0:
    ...
    def __get_exec_string(machine_info, cmd, tag, ts, obtain_stdout):
        ...
        command_str = '{python} {runner} {config}'.format(
            python=python_str,
            runner=cmd,
            config='{}/{}/config_{}.yml'.format(machine_info['staging_dir'], ts, tag))
    ...
    def __start_agg_job(self, config_agg_dict, agg_files, trial_info, ti, ts):
        ...
        agg_client = Runner.__start_ssh_connection(agg_ip,
                                                   22,
                                                   trial_info['agg_machine']['ssh_username'])
        print(f'Agg connection made to {agg_ip}')

        local_staging_dir = '{}/{}/trial{}'.format(trial_info['local_staging_dir'],
                                                   self.__exp_timestamp, ti)
        machine_staging_dir = '{}/{}'.format(trial_info['agg_machine']['staging_dir'], ts)

        # make remote staging dir
        status, outstr = Runner.__exec_command_sync(agg_client,
                                                    'mkdir -p {}'.format(machine_staging_dir))
        ...
        for supp_file in agg_files:
            Runner.__copy_to_server(
                str(supp_file),
                agg_client,
                f'{machine_staging_dir}/{supp_file.name}')
    ...
    def __start_party_job(self, config_party_dict, party_files, trial_info, ti, pi, ts):
        ...
        party_client = Runner.__start_ssh_connection(party_ip,
                                                     22,
                                                     trial_info['party_machines'][pi]['ssh_username'])
        ...
        status, outstr = Runner.__exec_command_sync(party_client,
                                                    'mkdir -p {}'.format(machine_staging_dir))
    ...
    def __display_status(self, exit_statuses, agg_stdout, config_party_dicts,
                         round_progress, party_progress, n_parties):
        """
            ...
            while '\n' in self.__agg_outstr:
                ...
                if self.__started_training:
                    if party_ref >= 0:
                        ...
                        self.__party_responses += [Runner.__get_party_from_log(
                            agg_outline, config_party_dicts)]
        ...
        if any(exit_statuses):
            sys.stdout.write(
                "\rAgg finished: {} & Parties finished: {}".format(
                    'yes' if exit_statuses[-1] else 'no',
                    [vi for vi, v in enumerate(exit_statuses[:-1]) if v]))
            sys.stdout.flush()
    ...
    def get_trial_info(self, exp_info, machines, trial_nr):
        ...
        if exp_info['shuffle_party_machines']:
            machine_perm_gen = Runner.__perm_generator(exp_info['party_machines'],
                                                       exp_info['n_parties'])
            machines_used = set()
            # TODO: don't perform this check if it's impossible
            while len(machines_used) < len(exp_info['party_machines']):
                exp_party_machines = tuple(
                    next(machine_perm_gen) for _ in range(n_trials))
                machines_used = set(
                    [item for sublist in exp_party_machines for item in sublist])
        else:
            ...
            exp_party_machines = tuple(
                tuple(exp_party_machines_one) for _ in range(n_trials))
        ...
        for pi in range(n_parties):
            trial_info['party_machines'] += [self.get_machine_info(
                exp_party_machines[trial_nr][pi], machines)]
    ...
    def __copy_logs_to_local(machine_staging_dir, client, local_staging_dir, tag):
        ...
        Runner.__copy_from_server(f'{machine_staging_dir}/stderr_{tag}.txt',
                                  client,
                                  f'{local_staging_dir}/stderr_{tag}.txt')
    ...
    def get_metrics_filepath(self):
        ...
        return '{}/{}/trial{}/{}.{}'.format(
            ...
            self.__config_party_dicts[0]['metrics_recorder']['output_type']).replace(
                'party0', 'party${id}')
    ...
    def __copy_metrics_to_local(self, party_clients):
        ...
        for pi, config_party_dict in enumerate(self.__config_party_dicts):
            ...
            metrics_output_filepath_local = Template(self.get_metrics_filepath()).substitute({
                'ts': self.__exp_timestamp,
                'trial': self.__trial_cur,
                'id': pi
            })

            # don't copy files if there was no metrics recording
            metrics_file_exists[pi] = Runner.__stat_on_server(
                party_clients[pi],
                metrics_output_filepath_remote)
    ...
    @staticmethod
    def stage_trial_files(ibmfl_dir, generated_files_dir, local_trial_dir, machine_trial_dir,
        ...
        # for party, val in proc_file_map.items():
            # print(party, val)
    ...
    def run_trial(self, trial_info, config_agg, config_parties, ts, ui_mode='nb'):
        ...
        # > agg
        if ui_mode == 'cli':
            if not isinstance(config_agg, str):
                config_agg = str(config_agg)
            ...
            config_agg_str = Template(config_agg).safe_substitute({
                'ts':          ts
            })
        else:
            print('Bad ui mode.')
            exit(0)
        if isinstance(config_agg_str, str):
            config_agg_dict = yaml.load(config_agg_str, Loader=yaml.Loader)
        else:
            config_agg_dict = config_agg_str

        # > parties
        self.__config_party_dicts = []
        for pi in range(trial_info['n_parties']):
            if ui_mode == 'cli':
                if not isinstance(config_parties, str):
                    config_parties = str(config_parties)
                ...
                config_party_str = Template(config_parties[pi]).safe_substitute({
                    'ts':          ts
                })
            else:
                print('Bad ui mode.')
                exit(0)
            self.__config_party_dicts += [yaml.load(config_party_str,
                                                    Loader=yaml.Loader
                                                   ) 
                                          if isinstance(config_party_str, str)
                                          else config_party_str]

        print("Trial has ID {}".format(ts))
        generated_files_dir = \
            str(Path(trial_info['local_staging_dir']).joinpath(self.__exp_timestamp))
        local_trial_dir = \
            Path(f'{trial_info["local_staging_dir"]}/{self.__exp_timestamp}/trial{self.__trial_cur}')
        local_trial_dir.mkdir(parents=True, exist_ok=True)
        staging_trial_dir = '{}/{}'.format(trial_info['agg_machine']['staging_dir'], ts)
        Runner.__dump_trial_metadata(trial_info,
                                     config_agg_dict,
                                     self.__config_party_dicts,
                                     self.__exp_timestamp,
                                     self.__trial_cur)

        # get key variables from config
        config_agg_dict['hyperparams']['global']['rounds'] = int(config_agg_dict['hyperparams']['global']['rounds'])
        config_agg_dict['hyperparams']['global']['num_parties'] = int(config_agg_dict['hyperparams']['global']['num_parties'])
        n_rounds = config_agg_dict['hyperparams']['global']['rounds']
        n_parties = config_agg_dict['hyperparams']['global']['num_parties']
        n_quorum = int(n_parties * config_agg_dict['hyperparams']['global']['perc_quorum'])

        if ui_mode == 'nb':
            # copy to trial dir
            proc_file_dict = Runner.stage_trial_files(trial_info['local_ibmfl_dir'],
                                                      generated_files_dir,
                                                      local_trial_dir,
                                                      staging_trial_dir,
                                                      config_agg_dict,
        ...
        # start agg
        agg_client, agg_handles = self.__start_agg_job(config_agg_dict,
                                                       proc_file_dict['agg'],
                                                       trial_info, self.__trial_cur,
                                                       ts)
        ...
        party_handles_list = []
        for pi, config_party_dict in enumerate(self.__config_party_dicts):
            party_client, party_handles = self.__start_party_job(config_party_dict,
                                                                 proc_file_dict[f'party{pi}'],
                                                                 trial_info, self.__trial_cur,
                                                                 pi, ts)
        ...
        party_progress = tqdm(total=n_quorum, desc="Party responses: ")
        while not all(exit_statuses):
            # display status
            self.__display_status(exit_statuses,
                                  agg_handles[1].channel, self.__config_party_dicts,
                                  round_progress, party_progress,
                                  n_parties)
    ...
    def get_experiment_output(self):
        ...
        offset_methods = {
                'off': ibmfl_postproc.offset_method_first,
                'del': ibmfl_postproc.offset_method_delta,
                'avg': ibmfl_postproc.offset_method_avg
            }
        dat = ibmfl_postproc.offset_vals(
            dat,
            [k for k,v in dat.items() if ':ts' in k],
            offset_methods)
    ...
    def call_postproc_fn(self):
        ...
        self.get_postproc_fn()(self.get_metrics_filepath(),
...
def get_config(folder):
    """
    Load configs from folder, must contain files "config_agg_tmpl.yml",
    "config_party_tmpl.yml", and "config_runner.yml".
    """
       
    with open(f'{folder}/config_agg_tmpl.yml', 'r') as config_agg_file:
        config_agg_str = yaml.load(config_agg_file.read(),
                                   Loader=yaml.Loader)
    with open(f'{folder}/config_party_tmpl.yml', 'r') as config_party_file:
        config_party_str = yaml.load(config_party_file.read(),
                                     Loader=yaml.Loader)
    with open(f'{folder}/config_runner.yml') as config_global_file:
        config_global = dict(yaml.load(config_global_file.read(), Loader=yaml.Loader))

    return config_agg_str, config_party_str, config_global


def changedict(d):
    """
    Recursively corrects dict types to contain floats and lists
    instead of nans and ndarrays.
    """
    if isinstance(d, dict):
        return {k: changedict(v) for k, v in d.items()}
    if isinstance(d, list):
        return [changedict(l) for l in d]
    if isinstance(d, np.ndarray):
        return changedict(d.tolist())
    if np.isnan(d):
        return -1
    return d
...
if __name__ == '__main__':
    ...
    tests = []
    big_metrics_dict = {}
    if "config_runner.yml" in os.listdir(sys.argv[1]):
        tests.append(sys.argv[1])
    else:
        for entry in os.listdir(sys.argv[1]):
            if os.path.isdir(f"{sys.argv[1]}/{entry}"):
                tests.append(f"{sys.argv[1]}/{entry}")
    for test in tests:
        with open(f'{test}/config_runner.yml') as config_global_file:
        ...
        for exp_info in experiments:
            with open(f'{test}/config_agg_tmpl.yml', 'r') as config_agg_file:
                config_agg_str = config_agg_file.read()
            with open(f'{test}/config_party_tmpl.yml', 'r') as config_party_file:
                config_party_str = config_party_file.read()
    #         config_parties = []
    #         for pi in range(exp_info['n_parties']):
    #             with open(
    #                 '{}/config_party{}.yml'.format(config_global['local_conf_dir'], pi), 'r'
    #             ) as config_party_file:
    #                 config_parties += [config_party_file.read()]
            ...
            if metrics_dict:
                ...
                big_metrics_dict[test] = metrics_dict


    with open(sys.argv[1] + "/metrics.json", "w") as outfile:
        outfile.write(json.dumps(changedict(big_metrics_dict), indent=4))
